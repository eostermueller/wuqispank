...
public class MoreComplicatedSqlTest {
	...
	@Test
	public void canParseInnerJoin() throws SqlParseException {
		String sql = "select TAL1.FOO AS CAL1 from MYSCHEMA.TAB1 TAL1 inner join MYSCHEMA.TAB2 TAL2 on TAL1.COL1=TAL2.COL2";
		...
		ITable tab1 = model.findTable("TAB1");
		assertEquals("Coudln't find table name in INNER JOIN", "tab1", tab1.getName().toLowerCase());
		assertEquals("Coudln't find table alias in INNER JOIN", "tal1", tab1.getAlias().toLowerCase());
