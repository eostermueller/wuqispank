...
public class Result implements Serializable {
	public static final HealthCheckType INFLUXDB = HealthCheckType.INFLUXDB;
	public static final HealthCheckType INTRACE = HealthCheckType.INTRACE;
	public static final HealthCheckType GRAFANA = HealthCheckType.GRAFANA;
	public enum HealthCheckType { INFLUXDB, INTRACE, GRAFANA }
	...
	private HealthCheckType myType = null;
    ...
    private void parseMessage() {
    	if (this.getMessage() != null && this.getMessage().trim().length() != 0) {
    		String args[] = this.getMessage().split(this.DELIMITER);
    	
    		if (args.length ==3) {
    	    	this.myType = HealthCheckType.valueOf(args[0]);
    	    	this.host = args[1];
    ...
    public static String format(HealthCheckType type, String hostName, int port) {
		return type.toString() + DELIMITER + hostName + DELIMITER + port; 
    }
